...
var s = (function () {
  ...
  s.getActionName = function (action, m) {
    return action;
  };
  ...
  s.setToken = function (token, profile) {
    s._token = token;
    if (s._sessionStorageAllowed) {
      try {
        if (token) {
  ...
  s.getToken = function (profile) {
    ...
    if (!tk) {
      if (s._sessionStorageAllowed) {
        try {
          var token_1 = sessionStorage.getItem('token');
          if (token_1) {
            tk = token_1;
          }
        }
...
function getActionName(action, m) {
