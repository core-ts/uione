...
var storage = (function () {
  ...
  storage.getDateFormat = function () {
    ...
    if (user) {
      if (user.dateFormat) {
        var x_1 = user.dateFormat;
        return (storage.moment ? x_1.toUpperCase() : x_1);
      }
      else {
        lang = user.language;
      }
    ...
    var x = (locale ? locale.dateFormat : exports.enLocale.dateFormat);
  ...
  storage.getLocale = function () {
    var l = storage.locale();
    if (l) {
      var lang = storage.getLanguage();
      var locale = l.getLocale(lang);
      if (locale) {
        return locale;
      }
    }
